package Gun18;

import java.util.Scanner;

public class _01_Java2DArrays {
    public static void main(String[] args) {


        int sayi=0; // 1 tane sayi
        int []dizi=new int [100];//100 sayı

        int []ders1Notlari=new int[50];//50 kişilik sınıfın 1. dersin notları
        int []ders2Notlari=new int[50];
        int []der3Notlari=new int[50];//0-49 arası index değişiyor

        int[][]tumDersNotlari=new int[3][50];//***3 tane ders ve 50 tane öğrenci,** hafızada 150 tane sayi saklar
                                            //3 satır 50 sütun
                                            // her satır 50 tane sayi
                                            //0. satır , 1.satır,2.satır
                                           //Satır:0,1,2     sütun:0,1,2,3...49
        ders1Notlari[0]=80;// tek boyutlu dizinin ilk(0) elemanına 80 değerini atadım
        tumDersNotlari[0][0]=80;//2 boyutlu dizide 0. satırın,0.sütununa 80 değeri atandı

        System.out.println("tum ders notları[0][0]:"+tumDersNotlari[0][0]);
        Scanner oku=new Scanner(System.in);
        tumDersNotlari[0][0]=oku.nextInt();






    }
}
